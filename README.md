...
- 本地 Agent companion：从终端/PowerShell 项目的 git diff 生成可导入学习地图的复盘 JSON
- 能力雷达：按编程、写作、演示、研究、产品、工具、数学等维度汇总学习点
- AI 依赖度自评：记录“没有 AI 我能完成多少”
- 错题本：保存 AI 可疑点、错误点和你手动标记的问题
- 同类任务成长线：把重复出现的知识点串起来看进步
- 复盘质量评分：记录哪类复盘真的有用
- 学习卡片导出：把知识点和微测试导出为 Markdown
...
- 顶部能力总览会显示能力雷达、AI 依赖度、同类任务成长线和错题本。
- 在复盘面板里可以记录“没有 AI 能完成多少”、给复盘评分，并把可疑点加入错题本。
- 点击“导出学习卡片”可以生成 Markdown 卡片，后续可导入 Anki/Notion 或自己复习。

## 本地 Agent / PowerShell 任务

浏览器扩展不能直接读取 PowerShell、Codex CLI、VS Code 终端或桌面 IDE 里的 agent 内容。对这类任务，使用本地 companion 脚本：

```powershell
node .\scripts\local-agent-review.js --task "修复 Gemini 识别和自动复盘" --cwd "D:\你的项目" --output .\afterai-agent-review.json
```

脚本会读取目标目录的 `git status`、`git diff --stat`、`git diff` 和最近提交，生成 `afterai-agent-review.json`。打开学习地图后，在左侧“本地 Agent”区域导入这个 JSON。

如果想让脚本直接生成知识点复盘，先配置环境变量：

```powershell
$env:AFTERAI_PROVIDER="openai"
$env:AFTERAI_BASE_URL="https://api.example.com/v1"
$env:AFTERAI_MODEL="your-model"
$env:AFTERAI_API_KEY="your-key"
node .\scripts\local-agent-review.js --task "本地 agent 完成的任务"
```

支持的 `AFTERAI_PROVIDER`：`openai`、`gemini`、`claude`、`tgi`、`cohere`。
